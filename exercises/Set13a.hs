...
checkNumber = checkStrings $ check (all isAlpha)
...
checkCapitals = checkStrings $ check (isUpper . head)

checkStrings :: (String -> Maybe String) -> (String, String) -> Maybe (String, String)
checkStrings f (for, sur) = do
  for' <- f for
  sur' <- f sur
  return (for', sur')
...
occurrences = (>> fmap length get) . mapM count
