...
sums i = go 0 i
  where
    go _ 0 = []
    go cum count = (cum + cur) : go (cum + cur) (count - 1)
      where
        cur = i - count + 1
...
indexDefault (_ : rest) i def = indexDefault rest (i - 1) def
...
sorted (first : second : rest)
  | sorted [first, second] = sorted (second : rest)
  | otherwise = False
...
sumsOf xs = go 0 xs
  where
    go _ [] = []
    go sum (first : rest) = newSum : go newSum rest where newSum = sum + first
...
merge :: [Int] -> [Int] -> [Int]
merge [] [] = []
merge (i : rest) [] = i : merge rest []
merge [] (i : rest) = i : merge rest []
merge (x : restX) (y : restY)
  | x <= y = x : merge restX (y : restY)
  | otherwise = y : merge (x : restX) restY
...
mymaximum bigger initial (first : rest) = mymaximum bigger currentMax rest
  where
    currentMax = if bigger first initial then first else initial
...
map2 _ [] _ = []
map2 _ _ [] = []
map2 f (x : restX) (y : restY) = f x y : map2 f restX restY
...
maybeMap _ [] = []
maybeMap f (x : rest) = go (f x)
  where
    go (Just x) = x : go Nothing
    go Nothing = maybeMap f rest
