...
visit maze place = do
  seen <- get
  unless (place `elem` seen) $ do
    modify (place :)
    forM_ (neighbors place maze) (visit maze)

neighbors :: String -> [(String, [String])] -> [String]
neighbors = (Data.Maybe.fromMaybe [] .) . lookup
...
path maze place1 place2 = place2 `elem` seen
  where
    (_, seen) = runState (visit maze place1) []
...
findSum2 ks ns = do
  ...
  j <- ks
  let n = i + j
  if n `elem` ns then return (i, j, n) else mempty
...
instance Monad SL where
  (SL f) >>= g = SL h
    where
      h s =
        let (a, as, al) = runSL (SL f) s
            (b, bs, bl) = runSL (g a) as
         in (b, bs, al ++ bl)
