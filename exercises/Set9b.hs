...
prettyPrint n qs =
  concat
    [ cell ++ lineEnding
      | x <- [1 .. n],
        y <- [1 .. n],
        let lineEnding = if y == n then "\n" else "",
        let cell = if (x, y) `elem` qs then "Q" else "."
    ]
...
canAttack a b = or [f a b | f <- [sameRow, sameCol, sameDiag, sameAntidiag]]

danger :: Candidate -> Stack -> Bool
danger c = any $ canAttack c
...
fixFirst n ((x, y) : qs)
  | x > n || y > n = Nothing
  | danger (x, y) qs = fixFirst n $ nextCol (x, y) : qs
