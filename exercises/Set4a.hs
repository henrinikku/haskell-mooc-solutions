...
allEqual [] = True
allEqual [_] = True
allEqual (first : second : rest) = first == second && allEqual (second : rest)
...
longest :: Ord a => [[a]] -> [a]
longest = maximumBy compareLengthThenFirstElem

compareLengthThenFirstElem :: Ord a => [a] -> [a] -> Ordering
compareLengthThenFirstElem [] [] = EQ
compareLengthThenFirstElem [] _ = LT
compareLengthThenFirstElem _ [] = GT
compareLengthThenFirstElem (a : as) (b : bs)
  | length as == length bs = compare b a
...
freqs = foldr helper Map.empty
  where
    helper item counts =
      let f _ = Just (Map.findWithDefault 0 item counts + 1)
       in Map.alter f item counts
...
maxIndex xs = fst $ maximumBy (comparing snd) (assocs xs)
