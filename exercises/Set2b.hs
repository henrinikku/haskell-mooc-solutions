...
isPrime n
    | n `elem` [0, 1] = False 
    | n `elem` [2, 3] = True 
    | otherwise = isPrime' n 2 (round (sqrt (fromIntegral n)) + 1)

isPrime' :: Integer -> Integer -> Integer -> Bool 
isPrime' n current limit = 
    not (divisibleBy n current)
    && (limit <= current || isPrime' n (current + 1) limit)
