...
step AddingFlour AddSugar = NeedsMixing
step AddingSugar AddFlour = NeedsMixing
step NeedsMixing Mix = NeedsBaking
step NeedsBaking Bake = Finished
...
toNonEmpty xs = head xs :| tail xs

toList :: NonEmpty a -> [a]
toList (x :| xs) = x : xs

reverseNonEmpty :: NonEmpty a -> NonEmpty a
reverseNonEmpty = toNonEmpty . reverse . toList
...
instance Operation2 Add2 where
  ...
  show2 (Add2 i j) = show1 (Add1 i j)

instance Operation2 Subtract2 where
  compute2 (Subtract2 i j) = i - j
  show2 (Subtract2 i j) = show1 (Subtract1 i j)

instance Operation2 Multiply2 where
  compute2 (Multiply2 i j) = i * j
  show2 (Multiply2 i j) = show1 (Multiply1 i j)
...
passwordAllowed s (ContainsSome x) = not $ null $ intersect s x
...
render (Add x y) = wrapParens $ render x ++ "+" ++ render y
render (Multiply x y) = wrapParens $ render x ++ "*" ++ render y
