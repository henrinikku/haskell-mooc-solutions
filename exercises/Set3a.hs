...
addOneIfOdd :: Int -> Int
addOneIfOdd n = if even n then n else n + 1

firstHalf :: String -> String
firstHalf s = take (addOneIfOdd (length s) `div` 2) s
...
whileRight f x = whileRight' (f x) f x

whileRight' :: Either b a -> (a -> Either b a) -> a -> b
whileRight' (Right result) f _ = whileRight f result
whileRight' (Left result) _ _ = result
...
joinToLength len arr =
  [ joined
    | a <- arr,
      b <- arr,
      let joined = a ++ b,
      hasLength len joined
  ]
...
multiCompose :: [c -> c] -> c -> c
multiCompose [] = id
multiCompose [a] = a
multiCompose (a : b : rest) = multiCompose $ (a . b) : rest
...
interpreter commands = go 0 0 commands
  where
    go y x ("up" : rest) = go (y + 1) x rest
    go y x ("down" : rest) = go (y - 1) x rest
    go y x ("left" : rest) = go y (x - 1) rest
    go y x ("right" : rest) = go y (x + 1) rest
    go y x ("printX" : rest) = show x : go y x rest
    go y x ("printY" : rest) = show y : go y x rest
    go _ _ (_ : commands) = commands
    go _ _ [] = []
