...
greetText name = T.pack $ "Hello, " ++ clamp (T.unpack name) ++ "!"
  where
    clamp s
      | length s > 15 = take 15 s ++ "..."
      | otherwise = s
...
shout :: T.Text -> T.Text
shout s = T.unwords $ zipWith apply (cycle [T.toUpper, id]) (T.words s)
  where
    apply f = f
...
longestRepeat = go 0 . T.unpack
  where
    go res [] = res
    go res (x : xs) =
      go
        (max res $ length $ takeWhile (== x) (x : xs))
        $ dropWhile (== x) xs
